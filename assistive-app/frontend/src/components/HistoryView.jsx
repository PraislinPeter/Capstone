...
import {
  ...
  BarChart3, ArrowLeft, Brain, Download, TrendingUp, ListVideo,
...
function AnalysisPanel({ session, sessions, onSeek, currentVideoSecs, trendInsights, notes, onAddNote, onDeleteNote }) {
  ...
  // Fetch game analysis for the selected session
  useEffect(() => {
    if (!session?.id) { setGameData(null); return; }
    fetch(`http://localhost:8000/api/game-events/analysis/session/${session.id}`)
      .then(r => r.json())
      .then(d => { if (d && d.status === "ok") setGameData(d); else setGameData(null); })
  ...
  const tabs = [
    ...
    { id: 'game',         label: 'Game Progress', icon: <Brain size={12} /> },
          ...
          <div className="space-y-4">
            {!gameData ? (
              <div className="text-center py-8 text-slate-400">
                ...
                <p className="text-xs font-bold">No texture book data for this session</p>
              </div>
            ) : (
              <>
                {/* Summary row */}
                <div className="grid grid-cols-4 gap-2">
                  <div className="bg-emerald-50 rounded-lg p-2 text-center border border-emerald-100">
                    <p className="text-[8px] font-bold text-emerald-600 uppercase">Explored</p>
                    <p className="text-sm font-black text-emerald-700">{gameData.summary.textures_explored}/{gameData.summary.textures_total}</p>
                  </div>
                  <div className="bg-blue-50 rounded-lg p-2 text-center border border-blue-100">
                    <p className="text-[8px] font-bold text-blue-600 uppercase">Correct</p>
                    <p className="text-sm font-black text-blue-700">{gameData.summary.total_correct}/{gameData.summary.total_questions}</p>
                  </div>
                  <div className="bg-violet-50 rounded-lg p-2 text-center border border-violet-100">
                    <p className="text-[8px] font-bold text-violet-600 uppercase">Accuracy</p>
                    <p className="text-sm font-black text-violet-700">{gameData.summary.overall_accuracy}%</p>
                  </div>
                  <div className={`rounded-lg p-2 text-center border ${gameData.summary.break_requests > 0 ? 'bg-purple-50 border-purple-100' : 'bg-amber-50 border-amber-100'}`}>
                    <p className={`text-[8px] font-bold uppercase ${gameData.summary.break_requests > 0 ? 'text-purple-600' : 'text-amber-600'}`}>
                      {gameData.summary.break_requests > 0 ? 'Breaks' : 'Wrong'}
                    </p>
                    <p className={`text-sm font-black ${gameData.summary.break_requests > 0 ? 'text-purple-700' : 'text-amber-700'}`}>
                      {gameData.summary.break_requests > 0 ? gameData.summary.break_requests : gameData.summary.total_wrong}
                    </p>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Per-Texture Results</p>
                  <div className="space-y-1.5">
                    {Object.entries(gameData.textures).map(([id, s]) => (
                      <div key={id} className={`rounded-lg p-2.5 border ${s.touched ? 'bg-white border-slate-200' : 'bg-slate-50 border-slate-100 opacity-40'}`}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm">{s.emoji}</span>
                          <span className="text-[10px] font-bold text-slate-800 flex-1">{s.name}</span>
                          <span className={`text-[7px] font-bold uppercase px-1 py-0.5 rounded-full ${
                        ...
                        {s.touched && (
                          <div className="flex items-center gap-3 text-[9px] flex-wrap">
                            <span className="font-bold text-emerald-600">{s.questions_correct}/{s.questions_total} correct</span>
                            {s.questions_wrong > 0 && <span className="font-bold text-amber-600">{s.questions_wrong} wrong</span>}
                            {s.feeling && <span className="font-bold text-violet-600">{EMO_ICON[s.feeling] || ''} {s.feeling}</span>}
                          </div>
                        )}
                      </div>
                    ))}
                ...
                {/* Sensory profile */}
                <div>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Sensory Profile</p>
                  <div className="space-y-2">
                    {Object.entries(gameData.sensory_profile).map(([cat, data]) => {
                      const colors = CAT_COLORS[cat] || CAT_COLORS.neutral;
                      const pct = data.accuracy_pct || 0;
                      return (
                        <div key={cat} className={`rounded-lg p-2.5 border ${colors.bg} ${colors.border}`}>
                          <div className="flex justify-between mb-1">
                            <span className={`text-[9px] font-bold ${colors.text}`}>{data.label}</span>
                            <span className={`text-[8px] font-bold ${colors.text}`}>{data.textures_touched}/{data.textures_total}</span>
                          </div>
                          {data.questions_total > 0 && (
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[9px] font-black ${colors.text}`}>{pct}%</span>
                            </div>
                          )}
                        </div>
                      );
