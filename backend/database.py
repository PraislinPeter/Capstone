...
DATABASE_URL = "mysql+pymysql://root:Alisha_2004@localhost/emotion_db"
...
class SessionRecord(Base):
    ...
    game_events = relationship("GameEvent", back_populates="session", cascade="all, delete")  # ◀ NEW
...
# ╔══════════════════════════════════════════════════════════════════╗
# ║  NEW: GameEvent — logs every child interaction from the app      ║
# ║                                                                  ║
# ║  event_type values:                                              ║
# ║    "texture"          — child touched a texture                  ║
# ║    "texture_question" — answered a knowledge question            ║
# ║    "texture_feeling"  — reported how the texture made them feel  ║
# ╚══════════════════════════════════════════════════════════════════╝
class GameEvent(Base):
    ...
    event_type = Column(String(50))     # texture | texture_question | texture_feeling
    item_id = Column(String(50))        # texture ID (e.g., "felt", "sandpaper")
    question_id = Column(String(50))    # question ID (e.g., "fe_feel", "sp_use")
    expected = Column(String(50))       # correct answer ID
    selected = Column(String(50))       # child's answer ID
    correct = Column(Integer)           # 1 = correct, 0 = wrong, NULL = n/a
    attempts = Column(Integer)          # attempt number
    duration_ms = Column(Integer)       # touch duration in ms
